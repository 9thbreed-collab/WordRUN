...
const BANNER_SAFE_PX = 35; // space reserved for ad banner at bottom
   ...
   // --- Powerup icon map (for all inventory slots) ---
const POWERUP_ICONS: Record<string, string> = {
  lockKey: '🔑',
  bomb: '💣',     // future powerup example
  hint: '💡',     // future powerup example
};
...
private levelCompleted = false;
    ...
    private releasedLocks = new Set<number>();
     ...
     private hintBadgeEl!: HTMLSpanElement;
     private hintCount = 3;
     // Inventory powerup bar (nullable so it won't crash if not found)
private powerBarEl: HTMLDivElement | null = null;
private lockKeyBadgeEl: HTMLSpanElement | null = null;
     ...
     private hintCdEl!: HTMLDivElement;
     private livesOverlayEl!: HTMLDivElement;
     private livesCdEl!: HTMLDivElement;
          // Powerup (Lock Key) confirm overlay
          private powerupOverlayEl!: HTMLDivElement;
          private powerupUseBtnEl!: HTMLButtonElement;
          private powerupSkipBtnEl!: HTMLButtonElement;
...
if (starTimes) {
  ...
  this.noMovesHandled = false;      // (optional extra safety if you want it here too)
    ...
    document.body.dataset.land = String(landMeta.id);
       ...
       this.comboValue = (typeof prevCombo === 'number' && prevCombo >= T1)
         ? Math.min(COMBO_MAX, Math.max(prevCombo, COMBO_MIN))
         : COMBO_MIN;
       (window as any).__comboValue__ = this.comboValue;
...
this.initRuutCompanionDom();
       this.pointsEl = el.querySelector<HTMLSpanElement>('#points')!;
       this.clockEl = el.querySelector<HTMLSpanElement>('#clock')!;
       this.hintTextEl = el.querySelector<HTMLSpanElement>('#hintText')!;
       this.hintBtnEl = el.querySelector<HTMLButtonElement>('#hintBtn')!;
       this.hintBadgeEl = el.querySelector<HTMLSpanElement>('#hintBadge')!;
...
this.lockKeyBadgeEl = el.querySelector<HTMLSpanElement>('#lockKeyBadge')!;
       ...
       this.hintCdEl = el.querySelector<HTMLDivElement>('#hintCd')!;
       this.livesOverlayEl = el.querySelector<HTMLDivElement>('#overlayLives')!;
       this.livesCdEl = el.querySelector<HTMLDivElement>('#livesCd')!;
       this.comboEl = el.querySelector<HTMLDivElement>('#combo')!;
       this.comboFillEl = el.querySelector<HTMLDivElement>('#comboFill')!;
...
this.recapDiamondEl.textContent = "💎";
       ...
       this.powerupOverlayEl = el.querySelector<HTMLDivElement>('#overlayPowerup')!;
       this.powerupUseBtnEl = el.querySelector<HTMLButtonElement>('#powerupUseBtn')!;
       this.powerupSkipBtnEl = el.querySelector<HTMLButtonElement>('#powerupSkipBtn')!;
       
       // We no longer support closing via "X" – only the explicit buttons
       this.powerupUseBtnEl.onclick = (ev) => {
         ev.preventDefault();
         this.confirmUseLockKeyFromPrompt();
       };
       
       this.powerupSkipBtnEl.onclick = (ev) => {
         ev.preventDefault();
         this.confirmSkipLockKeyFromPrompt();
       };
      ...
      this.powerSlots = Array.from(
        this.powerBarEl.querySelectorAll<HTMLDivElement>('.powerSlot')
      );

      this.attachPowerBarHandlers();
      this.renderPowerBar();
      ...
      answers.forEach((w, i) => {
       ...
       const useHint = () => {
         if (this.hintCount > 0) {
           ...
           const clue = HINTS[cur.word] || 'short clue';
           this.hintTextEl.textContent = clue;
           this.hintCount -= 1; if (this.hintCount === 0) this.hintNextAt = Date.now() + GameplayScene.HINT_COOLDOWN_MS;
           this.updateHintUI();
         } else { this.openHintModal(); }
         focusTrapFocus();
       };
       this.hintBtnEl.onclick = (ev) => { ev.preventDefault(); useHint(); setTimeout(() => this.updateCaret(true), 0); };
       this.updateHintUI();
...
// Hint modal
el.querySelector<HTMLButtonElement>('#modalClose')!.onclick = () => this.closeHintModal();
this.modalOverlayEl.addEventListener('click', (ev) => {
  if (ev.target === this.modalOverlayEl) this.closeHintModal();
});
el.querySelector<HTMLDivElement>('#modalPlay')!.onclick = () => {
  this.closeHintModal();
  this.focusTrap.focus({ preventScroll: true });
};

el.querySelector<HTMLButtonElement>('#hintNoThanks')!.onclick = () => {
  this.closeHintModal();
  this.focusTrap.focus({ preventScroll: true });
};
...
document.addEventListener('visibilitychange', this.handleVisibility, false);
       ...
       this.lastComboUpdateMs = performance.now();
...
private showPreLevelPanel() {
  ...
  private initRuutCompanionDom() {
    if (!this.ruutEl) return;
  
    const applySkin = () => {
      const { skinKey } = getRuutCosmetic();
  
      // MVP mapping (add more skins later)
      ...
      this.ruutEl!.style.backgroundImage = `url('${url}')`;
    };
  
    applySkin();
    this.setRuutMood("idle");
  
    try { this.ruutUnsub?.(); } catch {}
    this.ruutUnsub = onProfileChanged(() => applySkin());
  }
  
  private setRuutMood(mood: "idle" | "happy" | "sad") {
    if (!this.ruutEl) return;
  
    this.ruutEl.classList.remove("idle", "happy", "sad");
    this.ruutEl.classList.add(mood);
  
    // Auto-return to idle after reactions
    const anyEl: any = this.ruutEl;
    window.clearTimeout(anyEl.__ruutTO);
  
    if (mood === "happy") {
      anyEl.__ruutTO = window.setTimeout(() => this.setRuutMood("idle"), 650);
    } else if (mood === "sad") {
      anyEl.__ruutTO = window.setTimeout(() => this.setRuutMood("idle"), 450);
    }
  }
...
private startLevelClock() {
  ...
  this.clockTick = window.setInterval(() => this.clockTickFn?.(), 250);
    ...
    private resumeLevelClock() {
     ...
     /* ---------- Hints/Lives ---------- */
     private updateHintUI() {
       if (this.hintCount > 0) { this.hintBadgeEl.style.display = 'inline-flex'; this.hintBadgeEl.textContent = String(this.hintCount); }
       else { this.hintBadgeEl.style.display = 'none'; }
     }
     private openHintModal() {
       this.modalOverlayEl.style.display = 'flex';
       this.modalOverlayEl.classList.add('show'); this.modalOverlayEl.setAttribute('aria-hidden','false');
       if (!this.hintNextAt) this.hintNextAt = Date.now() + GameplayScene.HINT_COOLDOWN_MS;
       clearInterval(this.hintCdTimer);
       this.hintCdTimer = window.setInterval(() => {
         const rem = this.hintNextAt! - Date.now();
         this.hintCdEl.textContent = rem > 0 ? `${this.formatMs(rem)} till next hint` : 'Hint ready';
         if (rem <= 0) clearInterval(this.hintCdTimer);
       }, 1000);
     }
     private closeHintModal() { this.modalOverlayEl.classList.remove('show'); this.modalOverlayEl.setAttribute('aria-hidden','true'); this.modalOverlayEl.style.display = 'none'; }
...
this.starMeterFillEl.style.width = `${fillW}px`;
     ...
     update() { if (this.isHardPaused) return;
      this.drainCombo(); }
     private updateComboVisuals(initial = false) {
       const pct = Math.max(COMBO_MIN, Math.min(COMBO_MAX, this.comboValue)) * 100;
       this.comboFillEl.style.width = `${pct}%`;
       if (!initial) (window as any).__comboValue__ = this.comboValue;
     }
     private drainCombo() {
       const now = performance.now();
       const dt = (now - this.lastComboUpdateMs) / 1000;
       this.lastComboUpdateMs = now;
   
       const val = this.comboValue;
       if (now < this.comboLockedUntil) {
         const remaining = (this.comboLockedUntil - now) / 1000;
         if (remaining > 0) {
           const toDrain = Math.max(0, val - COMBO_MIN);
           const step = (toDrain / remaining) * dt;
           this.comboValue = Math.max(COMBO_MIN, val - step);
       ...
       let rate = 0;
       if      (val >= T3) rate = (val - COMBO_MIN) / DRAIN_T3;
       else if (val >= T2) rate = (val - COMBO_MIN) / DRAIN_T2;
       else if (val >= T1) rate = (val - COMBO_MIN) / DRAIN_T1;
       else                rate = (val - COMBO_MIN) / 4;
   
       const newVal = Math.max(COMBO_MIN, val - rate * dt);
   
       const prevTier = this.getComboTier(val);
       const newTier  = this.getComboTier(newVal);
       if (prevTier === 3 && newTier < 3) this.comboLockedUntil = now + DRAIN_T3 * 1000;
   
       this.comboValue = newVal;
       this.updateComboVisuals();
     }
     private getComboTier(v = this.comboValue) {
       if (v >= T3) return 3;
       if (v >= T2) return 2;
       if (v >= T1) return 1;
       return 0;
  ...
  if (this.isSlotLocked(this.currentSlotIndex)) {
    ...
    return; // 🔴 IMPORTANT: don't proceed to guess-checking for a locked row
       ...
       if (guess === s.word) {
         ...
         // combo gain + points
         this.comboValue = Math.min(COMBO_MAX, this.comboValue + 0.35);
         this.updateComboVisuals();
         let add = BASE_POINTS_PER_WORD;
         if (this.comboValue >= T3) add += BOOST_T3;
         else if (this.comboValue >= T2) add += BOOST_T2;
         else if (this.comboValue >= T1) add += BOOST_T1;
         this.addPoints(add);
...
this.setRuutMood("happy");
       ...
       if (this.inputMode === 'wheel' && this.currentSlotIndex < this.slots.length) {
         setTimeout(() => this.mountWheelForCurrentWord(), 100);
             ...
             // === Inventory Powerup Bar: render all 3 slots consistently ===
  private renderPowerBar() {
    if (!this.powerSlots || this.powerSlots.length === 0) return;

    // For now, we only have lockKey as a real powerup.
    // Slot 0: lockKey; Slots 1 & 2: reserved for future powerups.
    ...
    this.powerSlots.forEach((slotEl, idx) => {
      ...
      const iconChar = POWERUP_ICONS[type] ?? '•';
      iconSpan.textContent = iconChar;
      wrap.appendChild(iconSpan);

      // Show quantity badge when we have more than one of that powerup
      if (count > 1) {
      ...
      // === Inventory Powerup Bar: click behavior ===
  private attachPowerBarHandlers() {
    if (!this.powerSlots || this.powerSlots.length === 0) return;

    // For now, slot 0 is the Lock Key slot.
    const lockSlot = this.powerSlots[0];

    if (lockSlot) {
      lockSlot.onclick = () => {
        this.useLockKeyFromInventoryClick();
      };
    }

    // Slots 1 and 2 (powerSlots[1], powerSlots[2]) are reserved
    // for future powerups. We leave them clickable but no-op for now.
    const p2 = this.powerSlots[1];
    const p3 = this.powerSlots[2];

    if (p2) {
      p2.onclick = () => {
        // future: usePowerupFromSlot(1)
      };
    }
    if (p3) {
      p3.onclick = () => {
        // future: usePowerupFromSlot(2)
      };
    }
  }


    // When the player taps the Lock Key slot (slot 0)
    private onLockKeyClicked() {
      if (this.lockKeyCount <= 0) {
        this.flashConfirm("No lock keys available", "#ffaa00");
        return;
      }

      // Look for ANY locked row (respecting releasedLocks)
      let lockedIndex: number | null = null;
      try {
        const state: any = getTrapStateSafe();
        if (state && state.lockedIndices && typeof state.lockedIndices.has === "function") {
          for (const idx of state.lockedIndices as Set<number>) {
            if (!this.releasedLocks.has(idx)) {
              lockedIndex = idx as number;
              break;
            }
          }
        }
      } catch (err) {
        console.warn("[TRAP] unable to inspect lock state in onLockKeyClicked", err);
      }

      if (lockedIndex === null) {
        this.flashConfirm("No locked words to unlock", "#ffaa00");
        return;
      }

      // 🔓 Consume one key and free that locked row
      this.lockKeyCount = Math.max(0, this.lockKeyCount - 1);
      this.releasedLocks.add(lockedIndex);
      this.refreshLockedFromTraps();

      // 🔐 persist to global inventory
      this.gameData.setLockKeys(this.lockKeyCount);

      // 🔁 re-render power bar with new count
      this.renderPowerBar();

      // Move caret to the newly freed row so the player can solve it
      this.currentSlotIndex = lockedIndex;
      this.updateCaret(true);
      ...
      // Re-render the power bar so the slot updates (badge / empty dim state)
      this.renderPowerBar();
    }
    ...
    private showPowerupPrompt(lockedIndex: number) {
      this.pendingLockIndex = lockedIndex;
  
      if (!this.powerupOverlayEl) return;
      this.powerupOverlayEl.style.display = 'flex';
      this.powerupOverlayEl.classList.add('show');
      this.powerupOverlayEl.setAttribute('aria-hidden', 'false');
    }
  
    private hidePowerupPrompt() {
      if (!this.powerupOverlayEl) return;
      this.powerupOverlayEl.classList.remove('show');
      this.powerupOverlayEl.setAttribute('aria-hidden', 'true');
      this.powerupOverlayEl.style.display = 'none';
    }
  
    private confirmUseLockKeyFromPrompt() {
      if (this.pendingLockIndex === null) {
        this.hidePowerupPrompt();
        return;
      }
      if (this.lockKeyCount <= 0) {
        // Somehow no keys anymore; fall back to normal fail
        this.hidePowerupPrompt();
    ...
    private confirmSkipLockKeyFromPrompt() {
      this.pendingLockIndex = null;
      this.hidePowerupPrompt();
      this.triggerNoMovesFail();
    }
      ...
      currentSlotIndex: this.currentSlotIndex,
          ...
          private handleNoAvailableMoves() {
            ...
            if (this.lockKeyCount > 0 && lockedIndex !== null) {
              this.showPowerupPrompt(lockedIndex);
              return;
            }
...
pts.push(`${this.ptrX + offX},${this.ptrY + offY}`);
     ...
     private teardown() {
      ...
      try { this.ruutUnsub?.(); } catch {}
       ...
       clearInterval(this.hintCdTimer); clearInterval(this.livesCdTimer);
       this.stopLevelClock();
       document.removeEventListener('visibilitychange', this.handleVisibility, false);
     }
   }
