...
export const initUIDom = (scene: Phaser.Scene, html: string): Phaser.GameObjects.DOMElement => {
  // Create an interactive DOM container that actually accepts taps/clicks
  const style = 'width:100%;height:100%;pointer-events:auto;position:absolute;top:0;left:0;';
  const dom = scene.add.dom(0, 0, 'div', style).setHTML(html);
  dom.setOrigin(0, 0);
  dom.setScrollFactor(0);

  // Optional: prevent touch scrolling on mobile
  const el = dom.node as HTMLDivElement;
...
/**
 * DOM mounting helper functions
 */
export function mountDom(scene: Phaser.Scene, html: string) {
  const style = 'position:absolute;top:0;left:0;width:100%;height:100%;pointer-events:auto;';
  const dom = scene.add.dom(0, 0, 'div', style).setHTML(html);
  dom.setOrigin(0, 0);
  const el = dom.node as HTMLDivElement;
  el.style.touchAction = 'manipulation';
...
export function basicColumn(content: string) {
  return `
  <div style="position:absolute;inset:0;display:flex;flex-direction:column;
              align-items:center;justify-content:center;gap:12px;
              font-family: ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto; color:#fff;">
    ${content}
  </div>`;
}
export function getSafeRect(w: number, h: number, padPct = 0.05) {
  const padX = Math.round(w * padPct);
  const padY = Math.round(h * padPct);

  // extra top padding for notches/status bars (simple, works well)
  const notchTop = Math.round(Math.max(0, (h * 0.02)));

  const left = padX;
  const right = w - padX;
  const top = padY + notchTop;
  const bottom = h - padY;
