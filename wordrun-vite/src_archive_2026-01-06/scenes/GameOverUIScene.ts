...
export class GameOverUIScene extends Phaser.Scene {
  ...
  async restartGame(): Promise<void> {
    ...
    // Check if there's a failure level to navigate to, otherwise restart current level
    try {
      const { LevelManager } = await import('../LevelManager');
      const failureLevelId = await LevelManager.getNextLevelOnFail(this.currentLevelKey!);
      
      if (failureLevelId) {
        // Navigate to failure level with MapTransition
        await LevelManager.navigateToNextLevel(this.currentLevelKey!, false, this.scene.manager);
      } else {
        // No failure level defined, restart current level
        this.scene.start(this.currentLevelKey!);
      }
    } catch (error) {
      console.error('Failed to handle game over navigation:', error);
      // Fallback: restart current level
      this.scene.start(this.currentLevelKey!);
    }
  }
